...
INVERTEXTO_TOKEN = '25214|MbBdmsUSawtkPNjwwahhqgxYjBQqRqGf'
...
def consulta_cep(cep_input):
    ...
    if len(cep_input) !=8:
        return jsonify({"error": "CEP inválido! Deve conter 8 digitos"})
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cursor.close()
            conn.close()
            return jsonify({"error": "Codigo não encontrada na API"}), 404

        else:
            cursor.close()
            conn.close()
            return jsonify({"error": "Erro interno do servidor"}), response.status_code


    #Se não existir no DB, consulta o API externa
